...
- Business rules:
  1. The candidate directory root MUST be `~/career/<candidate-name>/` everywhere in career agents, skills, commands, opencode.json, workflow.md, and docs.
  2. The literal string `~/carreira/` MUST no longer appear in career artifacts; any remaining references are either historical/archived content (resolved_issues.md, prioritization.md proposals) or outside scope.
  3. Directory structure inside the root (`hub.json`, `README.md`, `entradas/`, `curriculos/`, `analise-perfil.md`, `cartas/`) is unchanged.
  4. No behavior change — this is a path/string rename only. Commands keep their argument shape (`<candidate-dir>`).
  5. The subdirectory named `curriculos/` (plural, Portuguese) is out of scope for this issue — only the root rename is requested.
  6. Permission rules in opencode.json and agent frontmatter MUST be updated to `~/career/**`.
  7. `prioritization.md` proposals and `resolved_issues.md` archives are historical and NOT rewritten (existing convention); but any ACTIVE/forward-looking references in the same files that guide new work should be consistent where trivial.
- Acceptance criteria:
  1. No occurrence of `~/carreira/` remains in: agents/career/*.md, skills/career/**/SKILL.md, commands/ocf:cv-*.md, opencode.json, workflow.md (career section), and scripts/tests/test_cv.sh.
  2. `~/career/` is used consistently in all the above.
  3. `make test-scripts` passes (test_cv.sh references updated).
  4. Command usage examples (`/ocf:cv-hub ~/career/maria-silva`, etc.) updated.
  5. opencode.json permission rules updated to `~/career/**`.
- Tests:
  1. `grep -r "~/carreira/"` over agents/career/, skills/career/ (excl. vendor), commands/ocf:cv-*.md, opencode.json, workflow.md, scripts/tests/test_cv.sh → 0 matches.
  2. `grep -r "~/career/"` over the same → matches present in each of agents/career/*.md, skills/career/**/SKILL.md, commands/ocf:cv-*.md, opencode.json, workflow.md.
  3. `make test-scripts` → full suite passes (test_cv.sh non-ASCII-path test now uses ~/career/joão-silva/).
  4. Existing historical references in resolved_issues.md / prioritization.md remain untouched (assert diff excludes them).
- Suggested fix: sed/string-replace `~/carreira/` → `~/career/` across the listed in-scope files; update opencode.json permission rules; update test_cv.sh fixture path; verify with grep + make test-scripts. Note: the candidate folder name itself (`<candidate-name>`) is unchanged.
