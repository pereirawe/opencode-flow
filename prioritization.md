...
### Proposal 2026-09-02-1: Revamp da otimização do LinkedIn — objetivo de perfil, relatório de ações, banner, sync hub↔LinkedIn e análise integrada (issues 222–226)
- Priority: high
- Business value: Após treinamento de LinkedIn, o usuário (desempregado, buscando contratação rápida como Tech Leader) precisa que a suíte career vire um guia de ações orientado ao OBJETIVO do perfil (procurar emprego / conexões / vender serviços): headline literal com os nomes das vagas, Sobre com logros em bullets (padrão de perfil de alto desempenho fornecido), experiência com bullets de resultado quantificados, revisão de skills comparando LinkedIn vs hub e banner profissional gerado por IA com telefone/e-mail/redes respeitando a zona da foto de perfil. A análise geral (cv-optimize) deve trazer as melhorias de LinkedIn junto, priorizadas. Posicionamento errado (ex.: founder/CEO quando se procura emprego) hoje é possível e prejudica a conversão.
- Target sprint: next
- Description: Conjunto de 5 issues do setor career (registradas no known_issues.md como 222–226):
  1. **#222** Hub: campo `profile_objective` (job_search/connections/services_sales/personal_branding + target_role/note) no schema/hub — dita headline, sobre, descrições e skills. Coletado quando o usuário declara; nunca inventado.
  2. **#223** cv-linkedin revamp: relatório orientado por objetivo — headline literal com nomes de vaga/serviço, Sobre estruturado (gatilho + logros ✔ + blocos "lógica"/"prática" + CTA), experiência com bullets de resultado (só números reais do hub ou gap), revisão de skills (adicionar/promover/remover) quando houver lista real do LinkedIn; output em 7 seções.
  3. **#224** Banner do LinkedIn: direção visual do art-director (agents/design/art-director) + geração via skill vendor poster-design-generation (each::sense AI, EACHLABS_API_KEY), conteúdo real do hub (telefone/e-mail/redes/frase), zona segura da foto (canto inferior esquerdo), legível em desktop e mobile, comando ocf:cv-banner.
  4. **#225** Sync offline LinkedIn↔hub: leitura do export oficial (Download My Data — nunca scraping; OAuth só dá nome/e-mail/foto) + diff (headline, experiência, skills, idiomas, educação, certificações) com recomendações; alimenta 223/226.
  5. **#226** cv-optimize: `profile-analysis.md` ganha seção "Melhorias no LinkedIn" (banner/headline/sobre/experiência/skills) integrada ao plano de ações priorizado e consistente com o cv-linkedin.
  Base branch: main para todas. Ordem de implementação sugerida: 222 → 225 → 223 → 226 (224 paralela/independente), com coordenação de merge nos arquivos compartilhados (skills/career/*, standards/cv-analysis.md).
- Business rules: (registradas por issue no known_issues.md — 222–226). Regras transversais: nada é inventado (dados vêm do hub/export); LinkedIn nunca é raspado nem modificado (o usuário cola manualmente); objetivo do perfil vem do hub ou é perguntado/declarado explicitamente — nunca assumir founder/CEO para quem procura emprego; relatórios no idioma do usuário; output final do cv-linkedin sem `[INFERIDO]`; dados sensíveis fora; skills do LinkedIn só via export oficial ou lista fornecida.
- Stakeholders: william_pereira (candidato, autor do treinamento de LinkedIn), cv-linkedin, cv-optimize, cv-hub, design (art-director), devops/runtime (key do provedor de imagem)
- Rationale: Pedido direto do usuário após treinamento de LinkedIn, com exemplo concreto de estrutura de "Sobre" de alto desempenho. A queixa central: falta objetivo do perfil, headline genérica, sobre/experiência sem estrutura de logros, sem revisão de skills vs LinkedIn e sem banner — e a análise não traz as melhorias de LinkedIn junto. Decisões de discovery tomadas com o usuário: várias issues por tema; sync via export oficial (offline); geração de imagem via modelo de IA externo existente no setor de design (each::sense, skill vendor poster-design-generation); objetivo guardado como campo novo no hub; skills = comparar LinkedIn vs hub; melhorias de LinkedIn integradas ao profile-analysis.md; escopo LinkedIn-only (cv-tailor não muda).
- Dependencies: Issue #64/#73 (idioma/setor career em inglês — respeitar padrão); skill vendor poster-design-generation (eachlabs, já importada — reutilizada no #224); agentes de design (art-director — reuso no #224); issue #222 é pré-requisito de 223/226; #225 alimenta 223/226. Coordenar merge-order com issues em aberto que tocam skills/career e standards/cv-analysis.md.
- Proposed issue type: feat
