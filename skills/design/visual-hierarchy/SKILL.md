...
2. Large text (18px and above, or 14px bold) MUST be 3:1 minimum (WCAG AA).
...
5. The accent pair `#4f46e5` on white MUST be used only for text that passes 4.5:1; lighter accent tints MUST NOT carry text.
...
Three density modes exist; a surface MUST pick one and apply it consistently.

- Comfortable (default): row height 48px, card padding 24px, gaps 16px, `font-size: 14px`.
- Compact: row height 40px, card padding 16px, gaps 12px, `font-size: 14px` (12px for table cells).
- Dense: row height 32px, card padding 12px, gaps 8px, `font-size: 12px`; numeric mono with `tabular-nums` required.

1. A surface MUST declare its density mode and MUST NOT mix modes within one view.
...
- Breakpoints: 640, 768, 1024, 1280 (px) - identical to the reference library.
...
1. Every surface MUST declare its density mode.
