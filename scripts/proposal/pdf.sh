#!/usr/bin/env bash
# Generate a commercial proposal PDF from an HTML file (Proposal Design Standard).
...
# Mechanism mirrors scripts/cv/pdf.sh; the reference HTML is
# skills/business-ops/proposal-writer/templates/proposal.html.
set -euo pipefail
...
# Proposal Design Standard §2.5 mandates a page-number footer. Chrome renders
# the CSS @page @bottom-right margin box; LibreOffice's HTML import ignores
# margin boxes, so the fallback output silently has no page numbers. Warn so
# the degradation is explicit (never a silent non-conformance).
if [[ "$USED_ENGINE" == "libreoffice" ]]; then
  echo "warning: LibreOffice fallback used — CSS @page margin-box page numbers are not rendered; verify footer manually or use Chrome" >&2
fi
