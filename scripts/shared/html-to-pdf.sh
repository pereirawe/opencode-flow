#!/usr/bin/env bash
# Generic HTML -> PDF converter.
# Engine: Google Chrome headless --print-to-pdf (A4, page CSS controls margins).
# Fallback: LibreOffice (headless convert-to pdf) when Chrome is unavailable.
#
# Contract (stable for all consumers):
#   CLI:  html-to-pdf.sh <input.html> <output.pdf> [chrome|libreoffice]
#   Exit: 0 = success (non-empty PDF written)
#         1 = runtime failure or no PDF engine available
#         2 = usage error or invalid input (missing/unresolvable path)
#   Guarantees: encoding gate (UTF-8 + NUL) runs BEFORE any render; missing
#   output dir is created before realpath; zero partial artifact on any failure
#   path; the "PDF generated:" success line on stdout is a stable contract.
set -euo pipefail

# Usage: html-to-pdf.sh <input.html> <output.pdf> [<chrome|libreoffice>]
INPUT="${1:-}"
OUTPUT="${2:-}"
FORCE_ENGINE="${3:-}"

if [[ -z "$INPUT" || -z "$OUTPUT" ]]; then
  echo "Usage: html-to-pdf.sh <input.html> <output.pdf> [chrome|libreoffice]" >&2
  exit 2
fi
...
# PDF — e.g. a "·" separator (UTF-8 C2 B7) corrupted to "\x00b7" renders as
...
check_encoding() {
  ...
  echo "error: input HTML contains NUL bytes (corrupted characters) — fix before generating the PDF" >&2
