...
the pipeline (`ocf:cv-optimize`, `ocf:cv-tailor`): `analise-perfil.md`,
`gap-analysis.md` and `inferencias.md`, plus their HTML/PDF renders. Every
...
Resolution order (per `standards/locale.md`, with the input language as the
first authority):
...
Applies to `analise-perfil.md`, `gap-analysis.md`, `inferencias.md` and their
HTML/PDF renders. The tailored RESUME is the exception: it follows the job
offer's language (cv-tailor rule), not this rule.

Protocol tokens are NEVER translated or localized — they stay as-is in any
report language:
...
### 3.1 `analise-perfil.md` (cv-optimizer)

Output: `~/career/<candidate-name>/analise-perfil.md`
...
Output: `~/career/<candidate-name>/curriculos/<job-slug>/gap-analysis.md`
...
Output: `~/career/<candidate-name>/curriculos/<job-slug>/inferencias.md`
...
2. **Allowed in internal analysis artifacts only** — hub.json,
   analise-perfil.md, gap-analysis.md, inferencias.md (and their HTML/PDF
...
`skills/career/cv-optimizer/templates/analise-perfil.html`. The cv-optimizer
flow copies it to `~/career/<candidate-name>/analise-perfil.html` and adapts
the CONTENT (never the CSS), then renders the PDF with
`bash $SCRIPTS_DIR/cv/pdf.sh analise-perfil.html analise-perfil.pdf`.
