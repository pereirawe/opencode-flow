...
1. Use the registry entries already loaded in context from `~/.config/opencode/agents/devs/REGISTRY.md`.
2. Inspect project-local agents first: `.opencode/agents/devs/*.md`.
3. Then inspect global agents: `~/.config/opencode/agents/devs/*.md`.
4. Specialized implementation agents are named exactly as the registry entries, for example `devs/golang` and `devs/python`.
...
6. Treat a Go module as a strong routing signal: if `go.mod` exists anywhere above the issue root, or the issue `Location:` is in `.go` code, prefer `devs/golang`.
7. If multiple specialized agents match, choose the strongest match and use that exact `devs/<language>` name in the Task tool.
...
- When a specialized agent matches, implementation must be handled by that `devs/*` agent, not by you.
