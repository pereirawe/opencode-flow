...
- **After creating the MR, update `known_issues.md`** with `- PR: #<n>`
  in the issue entry
- After creating the MR, trigger **Anderson** (`development/anderson.md`) to post
  user-feedback comment. If Anderson fails (gh/glab not installed, error), log
  and continue — non-blocking.
- After Anderson completes, delegate closure to the **Close Requester** agent,
