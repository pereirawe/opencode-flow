...
- **Skip this phase when the issue is already promoted** — when invoked from
  `/ocf:develop` / `/ocf:develop-full` (which promote before delegating), the
  issue is already `Status: in-progress` with the feature branch
  `issue-<id>-<slug>` checked out. Detect this and start directly at Phase 7.
