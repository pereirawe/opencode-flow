...
5. **Invoke the router** with full issue context; it must pick the matching `devs/*` agent from the registry and only fall back to `developer` when no specialized match exists
